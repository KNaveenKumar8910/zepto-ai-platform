...
def run_ingestion():
    ...
    # Save exact 8 policy corpus documents to disk
    ...
    collection = client.create_collection(name="zepto_policies", metadata={"hnsw:space": "cosine"})
    
    # Local embedding via sentence-transformers (all-MiniLM-L6-v2)
    model = SentenceTransformer("all-MiniLM-L6-v2")
    ids = list(DOCUMENTS.keys())
    texts = [DOCUMENTS[k] for k in ids]
    ...
    print("Ingestion complete: 8 documents embedded into ChromaDB collection 'zepto_policies'.")
